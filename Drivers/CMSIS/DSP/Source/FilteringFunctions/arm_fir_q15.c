  ...
  uint32_t blockSize)
{
  ...
  while (blkCnt > 0U)
  {
    ...
    *__SIMD32(pStateCurnt)++ = *__SIMD32(pSrc)++;
    ...
    /* Read the third and forth samples from the state buffer: _x[n-N-1], _x[n-N-2] */
    ...
    while (tapCnt > 0U)
    {
      ...
      /* Read state _x[n-N-2], _x[n-N-3] */
      x2 = _SIMD32_OFFSET(px1);

      /* Read state _x[n-N-3], _x[n-N-4] */
